...
class TestShadowForkCreate:
    ...
    def test_create_falls_back_when_base_branch_missing(self, tmp_path):
        """Test that create falls back to another ref if the base branch is missing.

        `_resolve_base_ref` tries the requested branch, then origin/<branch>,
        main, origin/main, master, origin/master, and finally HEAD. A missing
        base branch is therefore recoverable, not fatal.
        """
        fork = ShadowFork(repo_root=tmp_path)
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (False, "", "fatal"),  # _ref_exists("nonexistent") -> missing
                (False, "", "fatal"),  # _ref_exists("origin/nonexistent") -> missing
                (True, "", ""),  # _ref_exists("main") -> found, used as base
                (True, "", ""),  # Check for uncommitted changes
                (True, "", ""),  # Create and switch to shadow branch
