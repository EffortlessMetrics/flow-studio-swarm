...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_checkout_fails(self, tmp_path):
        """Test that create fails if git checkout command fails."""
        fork = ShadowFork(repo_root=tmp_path)

        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "", ""),  # Verify base branch (inside _resolve_base_ref)
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal: git checkout failed"),  # Create branch fails
            ]
