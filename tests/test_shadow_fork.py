...
class TestShadowForkCreate:
    ...
    def test_create_success(self, tmp_path):
        ...
        # Patch _resolve_base_ref to simplify test and avoid git calls
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # To test failure, we simulate _resolve_base_ref working (returning something),
        # but the checkout failing.
        # Note: _resolve_base_ref itself is resilient, so we don't test its failure here,
        # but rather what happens if git checkout fails.
        ...
        with patch.object(fork, "_resolve_base_ref", return_value="HEAD"):
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, "", ""),  # Check for uncommitted changes
                    (False, "", "fatal: not a git repository"),  # Create/checkout fails
                ]
        ...
        with patch.object(fork, "_resolve_base_ref", return_value="main"):
            with patch.object(fork, "_run_git") as mock_git:
                ...
                mock_git.side_effect = [
                    ...
                    (True, "", ""),  # git add (checkpoint)
                    (False, "", ""),  # git diff (has changes)
                    (True, "", ""),  # git commit
                    (True, "abc123", ""),  # git rev-parse
                    (True, "", ""),  # Checkout main (bridge)
                    (True, "", ""),  # Merge
                    (True, "main", ""),  # Get current branch (cleanup)
                    (True, "", ""),  # Delete shadow branch
                ]
        ...
        with patch.object(fork, "_resolve_base_ref", return_value="feature-x"):
            with patch.object(fork, "_run_git") as mock_git:
                ...
                mock_git.side_effect = [
                    ...
                    (True, "", ""),  # git commit
                    (True, "abc123", ""),  # git rev-parse
                    (True, "", ""),  # Verify commit (rollback)
                    (True, "", ""),  # Hard reset
                    (True, fork.shadow_branch, ""),  # Get current branch (cleanup)
