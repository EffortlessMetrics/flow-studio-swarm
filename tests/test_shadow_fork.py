...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to avoid internal git calls and return a specific ref
        with patch.object(fork, "_resolve_base_ref", return_value="nonexistent"):
            with patch.object(fork, "_run_git") as mock_git:
                ...
                # Create hooks directory for the test (needed for block_upstream_push call in exception handler?)
                # Actually, checkout fails before block_upstream_push.
