...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to simulate missing branch check inside create() logic
        # ShadowFork.create() does: _resolve_base_ref -> rev-parse -> fails
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal: ambiguous argument"),  # verify_ref(base_branch) fails
            ]

            # We need to mock _resolve_base_ref to avoid the actual git call
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        import logging
        caplog.set_level(logging.WARNING)  # The warning is logged at WARNING level
        fork = ShadowFork(repo_root=tmp_path)

        # Mock _resolve_base_ref to skip the git rev-parse calls
