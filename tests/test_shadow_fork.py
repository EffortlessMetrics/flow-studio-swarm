...
class TestShadowForkCreate:
    ...
    def test_create_falls_back_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                ...
                (False, "", "fatal"),  # Base branch (nonexistent)
                (False, "", "fatal"),  # origin/nonexistent
                (False, "", "fatal"),  # main
                (False, "", "fatal"),  # origin/main
                (False, "", "fatal"),  # master
                (False, "", "fatal"),  # origin/master
                # HEAD fallback succeeds without git call in loop, but loop structure calls _ref_exists(HEAD) -> True
                (True, "HEAD", ""),  # Verify HEAD exists (last resort)
                # Check for uncommitted changes
                (True, "", ""),
                # Create and switch to shadow branch
                (True, "", ""),
                # Install push guard (rev-parse)
                (True, "", ""),
            ]
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "main", ""),  # Verify base branch exists (default "main")
