...
class TestShadowForkCreate:
    ...
    def test_create_uses_fallback_if_base_missing(self, tmp_path, caplog):
        """Test that create falls back to HEAD if base branch doesn't exist."""
        caplog.set_level(logging.INFO)
        fork = ShadowFork(repo_root=tmp_path)

        # Mock _resolve_base_ref to simulate fallback to HEAD
        with patch.object(fork, "_resolve_base_ref", return_value="HEAD") as mock_resolve:
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    # _resolve_base_ref handled by mock
                    (True, "", ""),  # Check for uncommitted changes
                    (True, "", ""),  # Create and switch to shadow branch
                ...
                # Should have logged that it's using fallback
