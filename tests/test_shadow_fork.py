...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Mock _resolve_base_ref to simulate base branch resolution failure
        # Since _resolve_base_ref typically falls back to HEAD, we need to mock it
        # or the behavior around it to trigger a failure.
        # However, create() only fails if checkout fails.
        # To strictly test the failure path, we'll mock _run_git to fail on checkout.
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Patch _resolve_base_ref to avoid internal git calls consuming side_effect
        with patch.object(fork, "_run_git") as mock_git, \
            ...
            mock_git.side_effect = [
                ...
                # (True, "", ""),  # Verify base branch exists (skipped due to patch)
                (True, "", ""),  # Create and switch to shadow branch
            ]
