        ...
        with patch.object(fork, "_run_git") as mock_git:
            def git_side_effect(cmd, **kwargs):
                if cmd[:2] == ["rev-parse", "--abbrev-ref"]:
                    ...
                    return (True, "", "")
                elif cmd[:2] == ["show-ref", "--verify"]:
                    if cmd[2] in ["nonexistent", "refs/heads/nonexistent"]:
                        return (False, "", "fatal")
                    return (True, "", "")
                elif cmd[:2] == ["checkout", "-b"]:
                    # When git checkout -b tries to use nonexistent as base ref, it fails
            ...
            with pytest.raises(RuntimeError, match="Failed to create shadow branch|does not exist|fatal"):
