...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Mock _resolve_base_ref directly to avoid brittle sequence matching
        # Return a ref that we will simulate failing in _run_git
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Ensure we capture logs at WARNING level
        import logging
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                ...
                (True, "", ""),  # Verify base branch exists (via _resolve_base_ref if not mocked)
                (True, "", ""),  # Create and switch to shadow branch
            ...
            # Mock _resolve_base_ref to simplify the git call sequence
            with patch.object(fork, "_resolve_base_ref", return_value=True):
                fork.create()
