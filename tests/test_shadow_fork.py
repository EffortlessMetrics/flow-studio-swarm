...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to return 'nonexistent' directly, bypassing the logic
        # that would otherwise fallback to HEAD. This forces checkout to attempt using
        # the missing branch.
        with patch.object(fork, "_run_git") as mock_git, \
            ...
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal: branch 'nonexistent' does not exist"),  # Create and switch (fails)
            ]
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Patch _resolve_base_ref to simplify the test interaction
        with patch.object(fork, "_run_git") as mock_git, \
