        ...
        with patch.object(fork, "_run_git") as mock_git:
            def git_side_effect(cmd, **kwargs):
                ...
                if cmd == ["status", "--porcelain"]:
                    return (True, "", "")
                if cmd and cmd[0] == "rev-parse" and cmd[1] == "--verify":
                    # For _resolve_base_ref, we simulate everything failing until HEAD
                    if cmd[2] == "HEAD":
                        return (True, "HEAD", "")
                    return (False, "", "fatal: Needed a single revision")
                if cmd == ["checkout", "-b", fork.shadow_branch, "HEAD"]:
                    return (False, "", "fatal: base branch HEAD does not exist")
                return (True, "", "")

            mock_git.side_effect = git_side_effect

            with pytest.raises(RuntimeError, match="does not exist|Failed to create shadow branch"):
