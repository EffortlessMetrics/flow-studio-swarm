...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (False, "", "fatal"),  # Base nonexistent doesn't exist
                ...
                (False, "", "fatal"),  # HEAD doesn't exist
            ]

            with pytest.raises(RuntimeError, match="Could not resolve a valid base branch. None of the fallback branches exist"):
