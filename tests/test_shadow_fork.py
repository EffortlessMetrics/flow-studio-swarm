...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Mock _resolve_base_ref to return a reference that causes checkout to fail
        # This isolates us from the internal logic of _resolve_base_ref
        with patch.object(fork, "_run_git") as mock_git, \
             patch.object(fork, "_resolve_base_ref", return_value="nonexistent") as mock_resolve:

            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal: does not exist"),  # Create and switch to shadow branch (fails)
            ]

            # Expect failure during checkout
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Mock _resolve_base_ref to return a valid base
        with patch.object(fork, "_run_git") as mock_git, \
             patch.object(fork, "_resolve_base_ref", return_value="main") as mock_resolve:
