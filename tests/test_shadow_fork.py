...
class TestShadowForkCreate:
    ...
    def test_create_success(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            # Note: _resolve_base_ref may make calls, but we can verify
            # the core flow by mocking the sequence we expect
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                # _resolve_base_ref calls:
                (True, "", ""),  # check 'main' exists
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref directly to avoid implementation details
        with patch.object(fork, "_resolve_base_ref", return_value="nonexistent"):
            with patch.object(fork, "_run_git") as mock_git:
                ...
                # Create hooks directory for the test (needed if checkout succeeded, but we fail before)
                (tmp_path / ".git" / "hooks").mkdir(parents=True)

                # Expect RuntimeError because git checkout failed
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Patch _resolve_base_ref directly
        with patch.object(fork, "_resolve_base_ref", return_value="main"):
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, " M file.txt", ""),  # Uncommitted changes exist (status --porcelain returns output)
