    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        def mock_git_side_effect(args, **kwargs):
            if args == ["rev-parse", "--abbrev-ref", "HEAD"]:
                return (True, "main", "")
            elif args == ["rev-parse", "--verify", "nonexistent"]:
                return (False, "", "fatal: Needed a single revision")
            elif args == ["rev-parse", "--verify", "main"]:
                return (False, "", "fatal: Needed a single revision")
            elif args == ["rev-parse", "--verify", "master"]:
                return (False, "", "fatal: Needed a single revision")
            elif args == ["status", "--porcelain"]:
                return (True, "", "")
            elif args[:2] == ["checkout", "-b"]:
                return (False, "", "fatal: Cannot update paths and switch to branch at the same time.")
            return (True, "", "")
