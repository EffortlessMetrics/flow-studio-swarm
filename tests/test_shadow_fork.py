...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        """Test that create fails if checkout fails (e.g. invalid base ref)."""
        fork = ShadowFork(repo_root=tmp_path)
        ...
        def mock_run_git(args, **kwargs):
            if args[0] == "rev-parse" and args[1] == "--abbrev-ref" and args[2] == "HEAD":
                return True, "main", ""
            if args[0] == "show-ref":
                # Pretend none of the candidate refs exist so it falls back to HEAD
                return False, "", ""
            if args[0] == "status":
                return True, "", ""
            if args[0] == "checkout" and args[1] == "-b":
                # Fail the actual branch creation
