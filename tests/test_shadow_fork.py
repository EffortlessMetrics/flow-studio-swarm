...
class TestShadowForkCreate:
    ...
    def test_create_falls_back_to_head_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            ...
            # 2. Try 'nonexistent' -> False
            # 3. Try 'origin/nonexistent' -> False
            # 4. Try 'main' -> False
            # 5. Try 'origin/main' -> False
            # 6. Try 'master' -> False
            # 7. Try 'origin/master' -> False
            # 8. Use HEAD (no git call needed as HEAD is fallback)
            # 9. Status check
            # 10. Checkout -b ... HEAD
            mock_git.side_effect = [
                (True, "main", ""),  # 1. Get current branch
                (False, "", ""),  # 2. nonexistent
                (False, "", ""),  # 3. origin/nonexistent
                (False, "", ""),  # 4. main
                (False, "", ""),  # 5. origin/main
                (False, "", ""),  # 6. master
                (False, "", ""),  # 7. origin/master
                (True, "", ""),  # 9. Status check
                (True, "", ""),  # 10. Checkout -b
            ]
            ...
            # Should fall back to HEAD (which is always valid)
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            ...
            # 2. _resolve_base_ref("main") -> _ref_exists("main")
            # 3. status check
            # 4. checkout
            mock_git.side_effect = [
                (True, "main", ""),  # 1. Get current branch
