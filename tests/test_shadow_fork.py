...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to return what we passed, simulating success there
        # but _run_git will fail when trying to checkout
        with (
            ...
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal: invalid reference: nonexistent"),  # Create and switch fails
            ]
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Explicitly configure logging capture
        caplog.set_level(logging.INFO)

        # Patch _resolve_base_ref to avoid internal git calls that consume mock iterators
        with (
