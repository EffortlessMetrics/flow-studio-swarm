...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to return the input, bypassing internal git calls
        # This prevents the loop in _resolve_base_ref from consuming mock_git calls
        with patch.object(fork, "_resolve_base_ref", side_effect=lambda x: x):
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, "", ""),  # Check for uncommitted changes
                    (False, "", "fatal: commit not found"),  # Create shadow branch fails
                ]

                with pytest.raises(RuntimeError, match="commit not found"):
