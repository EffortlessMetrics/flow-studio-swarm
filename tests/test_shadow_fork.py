...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            # We mock _ref_exists so we don't need so many mock_git side effects
            with patch.object(fork, "_ref_exists", return_value=False):
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, "", ""),  # Check for uncommitted changes
                    (False, "", "fatal: A branch named 'shadow' already exists"),  # Create shadow branch checkout
                ]

                # The fallback logic now falls back to HEAD.
                # If creating the branch fails, it raises RuntimeError.
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            with patch.object(fork, "_ref_exists", return_value=True):
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
