...
class TestShadowForkCreate:
    ...
    def test_create_falls_back_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (False, "", "fatal"),  # Verify base branch 'nonexistent' fails
                (False, "", "fatal"),  # Verify 'origin/nonexistent' fails
                (False, "", "fatal"),  # Verify 'main' fails
                (False, "", "fatal"),  # Verify 'origin/main' fails
                (False, "", "fatal"),  # Verify 'master' fails
                (False, "", "fatal"),  # Verify 'origin/master' fails
                # Falls back to HEAD
                (True, "", ""),  # Check for uncommitted changes
                (True, "", ""),  # Create and switch to shadow branch (using HEAD)
