...
class TestShadowForkCreate:
    ...
    def test_create_falls_back_if_base_branch_missing(self, tmp_path, caplog):
        ...
        # Mock _resolve_base_ref directly to avoid complicated _run_git mocking
        with patch.object(fork, "_resolve_base_ref", return_value="HEAD"):
            with patch.object(fork, "_run_git") as mock_git:
                ...
                assert "using 'head' instead" in caplog.text.lower()

    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        """Test that create warns about uncommitted changes."""
        fork = ShadowFork(repo_root=tmp_path)

        # Mock _resolve_base_ref to avoid it consuming mock_git side effects
