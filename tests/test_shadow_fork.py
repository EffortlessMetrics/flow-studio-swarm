    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            def git_side_effect(cmd, **kwargs):
                ...
                if cmd == ["rev-parse", "--verify", "origin/nonexistent"]:
                    return (False, "", "fatal: Needed a single revision")
                if cmd == ["rev-parse", "--verify", "main"]:
                    return (False, "", "fatal")
                if cmd == ["rev-parse", "--verify", "origin/main"]:
                    return (False, "", "fatal")
                if cmd == ["rev-parse", "--verify", "master"]:
                    return (False, "", "fatal")
                if cmd == ["rev-parse", "--verify", "origin/master"]:
                    return (False, "", "fatal")
                if cmd[0] == "checkout":
                    # simulate checkout failure if base branch fallback failed (i.e. we use HEAD here)
                    return (False, "", "fatal")
                return (True, "", "")
