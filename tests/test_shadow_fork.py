...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to return 'nonexistent' directly, simulating failure to resolve
        # This prevents internal git calls inside _resolve_base_ref and allows testing checkout failure
        with patch.object(fork, "_run_git") as mock_git, \
             patch.object(fork, "_resolve_base_ref", return_value="nonexistent"):
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal: branch 'nonexistent' not found"),  # Checkout fails
            ]
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Explicitly set logger level to capture logs
        import logging
        ...
        with patch.object(fork, "_run_git") as mock_git, \
            ...
            mock_git.side_effect = [
                ...
                (True, "", ""),  # Create and switch to shadow branch
                (True, "", ""),  # Install push guard (rev-parse)
            ]
