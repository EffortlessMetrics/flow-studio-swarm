...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (False, "", "fatal: Needed a single revision"),  # preferred
                (False, "", "fatal: Needed a single revision"),  # origin/preferred
                (False, "", "fatal: Needed a single revision"),  # main
                (False, "", "fatal: Needed a single revision"),  # origin/main
                (False, "", "fatal: Needed a single revision"),  # master
                (False, "", "fatal: Needed a single revision"),  # origin/master
                # HEAD is not checked via rev-parse, it just returns "HEAD"
                (True, "", ""),  # Check for uncommitted changes
                (False, "", "fatal"),  # Base branch doesn't exist (checkout -b)
            ]
