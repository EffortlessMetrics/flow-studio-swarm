...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to return base branch name if valid, or "HEAD" if not
        # To simulate failure, we'll patch it to just return the nonexistent name
        # so checkout fails, OR we can rely on _resolve_base_ref logic.
        # But _resolve_base_ref handles missing branches by falling back to HEAD.
        # If we want create() to fail with "does not exist", checkout must fail.
        # So we let _resolve_base_ref return the bad name (by mocking it)
        # and then let checkout fail.

        with patch.object(fork, "_run_git") as mock_git:
            # We also need to patch _resolve_base_ref to avoid internal git calls
            # that would consume our side_effect
            with patch.object(fork, "_resolve_base_ref", return_value="nonexistent"):
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    # _resolve_base_ref is mocked, so no git calls there
                    (True, "", ""),  # Check for uncommitted changes
                    (
                        False,
                        "",
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            # Patch _resolve_base_ref to avoid interfering git calls
            with patch.object(fork, "_resolve_base_ref", return_value="main"):
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    # _resolve_base_ref mocked
                    (
                        True,
                        " M file.txt",
                        "",
                    ),  # Uncommitted changes exist (status --porcelain)
...
class TestShadowForkPushGuard:
    ...
    def test_allow_push(self, tmp_path):
        ...
        assert (
            ...
            or PRE_PUSH_HOOK_MARKER not in hook_path.read_text(encoding="utf-8")
        )
