...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Mock _resolve_base_ref directly to fail, avoiding brittle git call sequence matching
        with patch.object(fork, "_resolve_base_ref", side_effect=RuntimeError("does not exist")):
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Mock _resolve_base_ref to return a valid commit hash
        with patch.object(fork, "_resolve_base_ref", return_value="abc1234"):
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, " M file.txt", ""),  # Uncommitted changes exist
                    # (True, "", ""),  # Verify base branch exists (skipped due to mock)
                    (True, "", ""),  # Create and switch to shadow branch
                    (True, "", ""),  # Push guard
                ]
