...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Mock _resolve_base_ref to return the missing branch name
        # This forces the checkout command to fail as expected by the test
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Mock _resolve_base_ref to return valid branch
        with patch.object(fork, "_resolve_base_ref", return_value="main"):
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
    ...
    def test_full_workflow_success(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            ...
            mock_git.side_effect = [
                ...
                (True, "", ""),  # Verify base branch (via _resolve_base_ref -> _ref_exists)
                (True, "", ""),  # Create shadow branch
            ]

            # Since we're not mocking _resolve_base_ref here (testing integration feel),
            # we need to ensure the side effect list is long enough OR mock it.
            # But the integration test is tricky with internal calls.
            # Let's mock _resolve_base_ref for consistency in this test suite,
            # as verifying internal git calls for resolution is brittle.
            with patch.object(fork, "_resolve_base_ref", return_value="main"):
                mock_git.side_effect = [
                   (True, "main", ""),  # Get current branch
                   (True, "", ""),  # Check uncommitted changes
