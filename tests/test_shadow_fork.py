...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Patch _resolve_base_ref to simulate missing branch error directly
        with patch.object(fork, "_resolve_base_ref", side_effect=RuntimeError("Base branch nonexistent does not exist")):
            with patch.object(fork, "_run_git") as mock_git:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, "", ""),  # Check for uncommitted changes
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        # Explicitly set log level to INFO to capture the message
        import logging
        caplog.set_level(logging.INFO)

        # Patch _resolve_base_ref to return a fixed SHA/ref
