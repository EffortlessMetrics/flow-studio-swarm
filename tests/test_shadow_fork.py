...
class TestShadowForkCreate:
    ...
    def test_create_falls_back_if_base_branch_missing(self, tmp_path):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (False, "", ""),  # Verify base branch (nonexistent)
                (False, "", ""),  # Verify origin/base branch
                (False, "", ""),  # Verify main
                (False, "", ""),  # Verify origin/main
                (False, "", ""),  # Verify master
                (False, "", ""),  # Verify origin/master
                # HEAD is assumed valid by default fallback logic without git call
                (True, "", ""),  # Check for uncommitted changes
                (True, "", ""),  # Create and switch to shadow branch
            ]
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            mock_git.side_effect = [
                (True, "main", ""),  # Get current branch
                (True, "hash", ""),  # Verify base branch exists (from _resolve_base_ref)
