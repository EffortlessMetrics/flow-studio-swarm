...
class TestShadowForkCreate:
    ...
    def test_create_fails_if_base_branch_missing(self, tmp_path):
        ...
        # Mock _resolve_base_ref to simulate fallback behavior or pass-through
        # However, the original test expected create() to raise RuntimeError.
        # But create() falls back to other branches if _resolve_base_ref returns them.
        # If we want to test failure, we need checkout to fail.

        with patch.object(fork, "_run_git") as mock_git:
            # We mock _resolve_base_ref to avoid the loop consuming side_effects
            with patch.object(fork, "_resolve_base_ref", return_value="nonexistent") as mock_resolve:
                mock_git.side_effect = [
                    (True, "main", ""),  # Get current branch
                    (True, "", ""),  # Check for uncommitted changes
    ...
    def test_create_warns_on_uncommitted_changes(self, tmp_path, caplog):
        ...
        with patch.object(fork, "_run_git") as mock_git:
            # Mock _resolve_base_ref to return 'main' directly
