...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Special case for esbuild JSON bundles which contain HTML string templates that we DO want to parse.
            if 'type="application/json" data-inline-source="flowstudio-js-bundle"' not in line:
                in_script = True
    ...
    # We need to deduplicate them to avoid tests complaining about uniqueness
    # just because the same HTML template is embedded inside a script
    # and the HTML file.
    unique_uiids = []
    seen = set()
