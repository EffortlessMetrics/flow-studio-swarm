...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    js_bundle_script_start = re.compile(r'<script type="application/json" data-inline-source="flowstudio-js-bundle">', re.IGNORECASE)
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Special case: do not treat our JS bundle as a generic script block to skip
            if js_bundle_script_start.search(line):
                in_script = False
            else:
                in_script = True
