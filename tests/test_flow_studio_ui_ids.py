...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Skip treating application/json template scripts as standard scripts
    ...
    # to prevent false duplicate errors in tests, especially due to inlined esbuild outputs
    seen = set()
    deduped_uiids = []
