...
class TestRunDetailModalUIIDs:
    ...
    def test_run_detail_modal_elements_have_uiids(self):
        ...
        # Expected run detail modal UIIDs authored in the static HTML shell
        # (fragments/60-modals.html). The modal *contents* - re-run button,
        # exemplar checkbox, events section - are rendered at runtime by
        # run_detail_modal.ts and are covered by TestDynamicUIIDs instead.
        expected_modal = [
...
class TestDynamicUIIDs:
    ...
    def test_rerun_button_uiid_in_run_detail_modal(self):
        ...
        assert "flow_studio.modal.run_detail.rerun" in content, (
            "run_detail_modal.js should render the re-run button with data-uiid"
        )
