...
class TestRunDetailModalUIIDs:
    ...
    def test_run_detail_rerun_button_has_uiid(self):
        ...
        # The re-run button is dynamically rendered via JS, so we read the JS source directly
        from pathlib import Path

        js_path = Path("swarm/tools/flow_studio_ui/js/run_detail_modal.js")

        # Fallback to src if js doesn't exist (e.g. before build)
        ...
        assert f'data-uiid="{uiid}"' in content, (
            f"Run detail re-run button missing data-uiid='{uiid}' in {js_path.name}. "
