...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    in_script = False
    script_start = re.compile(r'<script(?!.*type="application/json".*data-inline-source="flowstudio-js-bundle")\b', re.IGNORECASE)
    ...
    # Deduplicate UIIDs (preserving first occurrence)
    seen = set()
    deduped_uiids = []
