...
class TestRunDetailModalUIIDs:
    ...
    def test_run_detail_rerun_button_has_uiid(self):
        ...
        # Read from the JS source file because the element is dynamically rendered
        js_path = project_root / "swarm" / "tools" / "flow_studio_ui" / "js" / "run_detail_modal.js"

        # Fallback to .ts file if .js isn't compiled yet during test setup
        if not js_path.exists():
            js_path = project_root / "swarm" / "tools" / "flow_studio_ui" / "src" / "run_detail_modal.ts"
