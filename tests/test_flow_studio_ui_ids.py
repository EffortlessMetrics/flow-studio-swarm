...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Don't skip our injected template block that contains HTML fragments
        ...
        for match in pattern.finditer(line):
            ...
            # Deduplicate extracted uiids based on value (keep the first occurrence)
            if value not in [u[0] for u in uiids]:
