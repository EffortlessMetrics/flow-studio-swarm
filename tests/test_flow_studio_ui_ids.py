...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # esbuild inlines HTML into a data block which we still want to parse
    ...
    # Deduplicate while preserving line numbers (keep first occurrence)
    deduped = []
    seen_values = set()
    for value, line in uiids:
        if value not in seen_values:
            deduped.append((value, line))
