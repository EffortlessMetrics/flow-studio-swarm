...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    # Track whether we're inside a script tag, UNLESS it's an inline bundle
    in_script = False
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        # Skip lines inside script tags (except inline source blocks which contain HTML templates)
        if in_script:
            continue
