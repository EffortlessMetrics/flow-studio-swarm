...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Special case: don't skip the flowstudio-js-bundle script block,
            # because esbuild inlines HTML templates (with their data-uiid attributes) inside it.
    ...
    # Deduplicate extracted uiids while preserving line numbers to prevent false duplicate errors
    # in tests that check for uniqueness when templates are inlined by esbuild.
    seen = set()
    deduped_uiids = []
