...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Special exception: The flowstudio-js-bundle script contains inlined HTML templates
            # created by esbuild, which contain actual DOM attributes. We should not skip this block.
    ...
    # Deduplicate while preserving the first seen line number
    unique_uiids = []
    seen = set()
