...
def extract_uiids_from_html(html: str) -> List[Tuple[str, int]]:
    ...
    for line_num, line in enumerate(html.split("\n"), start=1):
        ...
        if script_start.search(line):
            # Special case: do not skip the application/json script block that contains esbuild templates
    ...
    # Deduplicate values while keeping the first occurrence line number
    unique_uiids = {}
    for value, line_num in uiids:
        if value not in unique_uiids:
            unique_uiids[value] = line_num

    return list(unique_uiids.items())
