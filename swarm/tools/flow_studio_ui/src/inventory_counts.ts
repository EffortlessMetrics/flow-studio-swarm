...
async function getFactsSummary(runId: string): Promise<FactsSummaryResponse> {
  return fetchJSON<FactsSummaryResponse>(
    `/api/runs/${encodeURIComponent(runId)}/facts/summary`
  );
}
...
function getMarkerColor(markerType: string): string {
...
function renderScoreboard(data: FactsSummaryResponse): string {
...
function renderFlowBreakdown(data: FactsSummaryResponse): string {
...
function renderDeltaView(data: FactsSummaryResponse): string {
...
export function renderCompactInventory(data: FactsSummaryResponse): string {
  const items = data.by_type.map((mc) => {
...
export function renderInventoryPanel(data: FactsSummaryResponse): string {
  return `
 ...
 */
export function getCurrentFactsData(): FactsSummaryResponse | null {
  return currentData;
}
 ...
 */
export async function updateInventoryPanel(
 ...
 */
export async function updateCompactInventory(
  container: HTMLElement,
  runId: string
): Promise<void> {
  const data = await loadFactsSummary(runId);
...
export function injectInventoryCSS(): void {
