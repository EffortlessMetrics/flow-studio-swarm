...
function renderDecisionCard(decision: DecisionSummary): string {
  return `
...
function renderDetourCard(detour: DetourSummary): string {
  return `
...
function renderVerificationCard(verification: VerificationSummary): string {
...
function renderSummaryBar(data: BoundaryReviewResponse): string {
...
function renderUncertaintyNotes(notes: string[]): string {
...
function renderSection(title: string, items: string[], collapsed = false): string {
...
export function renderBoundaryReviewPanel(data: BoundaryReviewResponse): string {
  ...
  return `
    <div class="boundary-review-panel" data-run-id="${data.run_id}" data-scope="${data.scope}">
      <div class="panel-header">
        ...
        <button class="toggle-expand" title="Toggle expand" aria-expanded="${isExpanded ? 'true' : 'false'}" aria-label="${isExpanded ? 'Collapse boundary review' : 'Expand boundary review'}">
          ${isExpanded ? "▼" : "▶"}
        </button>
      </div>
 ...
 */
export function getCurrentBoundaryData(): BoundaryReviewResponse | null {
  return currentData;
}
 ...
 */
export async function updateBoundaryReviewPanel(
...
export function injectBoundaryReviewCSS(): void {
