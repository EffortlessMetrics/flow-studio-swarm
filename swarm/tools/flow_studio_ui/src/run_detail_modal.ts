...
import { escapeHtml, formatDateTime, createModalFocusManager, type ModalFocusManager, createCopyButton, createQuickCommands } from "./utils.js";
  ...
  return `
    ...
    <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      ...
      <div style="display: flex; gap: 12px;">
        ...
        <button id="run-detail-close-btn" aria-label="Close run detail" class="fs-button-small" style="flex: 1;">
          Close
        </button>
      </div>
    </div>
...
function attachActionHandlers(modal: HTMLElement, runId: string, summary?: ExtendedRunSummary): void {
  ...
  if (boundaryToggle && boundaryContainer) {
    boundaryToggle.addEventListener("click", async () => {
      const btn = boundaryToggle as HTMLButtonElement;
      const container = boundaryContainer as HTMLElement;
      ...
      if (container.style.display === "none") {
        ...
        try {
          ...
          container.innerHTML = `<div class="fs-text-xs" style="color: #dc2626;">Failed to load boundary review: ${escapeHtml((err as Error).message || "Unknown error")}</div>`;
  ...
  if (eventsToggle && eventsContainer) {
    eventsToggle.addEventListener("click", async () => {
      const btn = eventsToggle as HTMLButtonElement;
      const container = eventsContainer as HTMLElement;
      ...
      if (container.style.display === "none") {
        ...
        try {
          ...
          container.innerHTML = `<div class="fs-text-xs" style="color: #dc2626;">Failed to load events: ${escapeHtml((err as Error).message || "Unknown error")}</div>`;
          btn.textContent = "Retry";
        } finally {
          btn.disabled = false;
        }
  ...
  if (wisdomToggle && wisdomContainer) {
    wisdomToggle.addEventListener("click", async () => {
      const btn = wisdomToggle as HTMLButtonElement;
      const container = wisdomContainer as HTMLElement;
