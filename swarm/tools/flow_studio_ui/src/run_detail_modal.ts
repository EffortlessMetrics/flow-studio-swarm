...
import { escapeHtml, formatDateTime, createModalFocusManager, type ModalFocusManager, createCopyButton, createQuickCommands, setupAsyncToggle } from "./utils.js";
...
let _callbacks: RunDetailCallbacks = {};
let _focusManager: ModalFocusManager | null = null;
 ...
 */
export function configure(callbacks: RunDetailCallbacks = {}): void {
 ...
 */
function initModalCloseHandlers(modal: HTMLElement): void {
 ...
 */
export async function showRunDetailModal(runId: string): Promise<void> {
  _currentRunId = runId;
  const modal = getOrCreateModal();
  ...
  try {
    const summary = await Api.getRunSummary(runId) as ExtendedRunSummary;
    if (contentEl) {
    ...
    if (contentEl) {
      contentEl.innerHTML = renderRunDetailError(runId, err as Error);
    }
  }
}
 ...
 */
export function closeRunDetailModal(): void {
  toggleModal(false);
  _currentRunId = null;
 ...
 */
export function renderRunDetailContent(runId: string, summary: ExtendedRunSummary): string {
 ...
 */
function renderRunDetailError(runId: string, error: Error): string {
  return `
    <h3 id="run-detail-modal-title" class="selftest-step-title">Run Details</h3>
 ...
 */
function renderFlowProgress(flows: Record<FlowKey, FlowStatusData> | undefined): string {
  if (!flows) {
 ...
 */
function renderTags(tags: string[] | undefined, isExemplar: boolean | undefined, runId: string): string {
 ...
 */
function getFlowStatusIcon(status: string): string {
  switch (status) {
    case "done":
    case "complete":
      return "\u2705";
 ...
 */
function attachActionHandlers(modal: HTMLElement, runId: string, summary?: ExtendedRunSummary): void {
  // Inject CSS for inventory and boundary components
  injectInventoryCSS();
  injectBoundaryReviewCSS();

  // Load inventory counts immediately (compact view)
  const inventoryContainer = modal.querySelector("#run-detail-inventory-container") as HTMLElement | null;
  ...
  const boundaryToggle = modal.querySelector("#run-detail-boundary-toggle") as HTMLButtonElement | null;
  const boundaryContainer = modal.querySelector("#run-detail-boundary-container") as HTMLElement | null;
  if (boundaryToggle && boundaryContainer) {
    setupAsyncToggle(boundaryToggle, boundaryContainer, async () => {
      ...
      onError: (err) => {
        boundaryContainer.innerHTML = `<div class="fs-text-xs" style="color: #dc2626;">Failed to load boundary review: ${escapeHtml((err as Error).message || "Unknown error")}</div>`;
      }
    });
  }

  // Exemplar checkbox
  const exemplarCheckbox = modal.querySelector("#run-detail-exemplar-checkbox") as HTMLInputElement | null;
  const exemplarStatus = modal.querySelector("#run-detail-exemplar-status") as HTMLElement | null;
  if (exemplarCheckbox) {
    exemplarCheckbox.addEventListener("change", async () => {
      ...
      try {
        ...
        if (exemplarStatus) {
          exemplarStatus.textContent = `Error: ${(err as Error).message || "Failed to update"}`;
          exemplarStatus.className = "fs-text-xs";
  ...
  if (rerunBtn) {
    rerunBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (errorArea) {
          errorArea.textContent = `Error: ${(err as Error).message || "Failed to start run"}`;
        }
      }
    });
  }
  ...
  const eventsToggle = modal.querySelector("#run-detail-events-toggle") as HTMLButtonElement | null;
  const eventsContainer = modal.querySelector("#run-detail-events-container") as HTMLElement | null;
  if (eventsToggle && eventsContainer) {
    setupAsyncToggle(eventsToggle, eventsContainer, async () => {
      ...
      loadingLabel: "Loading...",
      onError: (err) => {
        eventsContainer.innerHTML = `<div class="fs-text-xs" style="color: #dc2626;">Failed to load events: ${escapeHtml((err as Error).message || "Unknown error")}</div>`;
      }
    });
  }

  // Wisdom toggle button
  const wisdomToggle = modal.querySelector("#run-detail-wisdom-toggle") as HTMLButtonElement | null;
  const wisdomContainer = modal.querySelector("#run-detail-wisdom-container") as HTMLElement | null;
 ...
 */
function renderEventsTimeline(events: RunEvent[]): string {
...
function getEventKindClass(kind: string): string {
  if (kind.includes("error") || kind.includes("fail")) return "fs-text-error";
  if (kind.includes("start") || kind.includes("created")) return "fs-text-info";
  if (kind.includes("end") || kind.includes("complete")) return "fs-text-success";
  return "";
}
 ...
 */
function renderWisdomSummary(wisdom: WisdomSummary): string {
  const { summary, flows, labels, created_at } = wisdom;

  // Flow status rows
  const flowOrder: FlowKey[] = ["signal", "plan", "build", "gate", "deploy", "wisdom"];
 ...
 */
function getWisdomStatusDot(status: string): string {
 ...
 */
function getLoopInfo(flowData: { microloops?: number; test_loops?: number; code_loops?: number } | undefined): string {
  if (!flowData) return "";
  const parts: string[] = [];
  if (flowData.microloops) parts.push(`μ:${flowData.microloops}`);
  if (flowData.test_loops) parts.push(`T:${flowData.test_loops}`);
  if (flowData.code_loops) parts.push(`C:${flowData.code_loops}`);
  return parts.join(" ");
}
 ...
 */
function renderWisdomEmpty(runId: string): string {
  return `
