...
import { escapeHtml, createModalFocusManager, type ModalFocusManager } from "../utils.js";
...
const STATUS_STYLES: Record<FlowCompletionStatus, { icon: string; color: string; bgColor: string; borderColor: string }> = {
  VERIFIED: {
...
export class BoundaryReview {
  ...
  private focusManager: ModalFocusManager | null = null;
  private options: BoundaryReviewOptions;
  private resolvePromise: ((decision: BoundaryReviewDecision) => void) | null = null;
  private currentData: BoundaryReviewData | null = null;
   ...
   */
  async show(data: BoundaryReviewData): Promise<BoundaryReviewDecision> {
   ...
   */
  close(decision: BoundaryReviewDecision = "cancel"): void {
   ...
   */
  private render(data: BoundaryReviewData): void {
    ...
    const content = this.panel.querySelector<HTMLElement>(".boundary-review-panel");
    if (content) {
   ...
   */
  private renderSummarySection(data: BoundaryReviewData): string {
   ...
   */
  private renderArtifactsSection(data: BoundaryReviewData): string {
   ...
   */
  private renderRoutingSection(data: BoundaryReviewData): string {
   ...
   */
  private renderIssuesSection(data: BoundaryReviewData): string {
   ...
   */
  private renderActions(data: BoundaryReviewData): string {
 ...
 */
export function createBoundaryReview(options?: BoundaryReviewOptions): BoundaryReview {
 ...
 */
export function extractBoundaryReviewData(
  flowKey: FlowKey,
  flowTitle: string,
  status: FlowCompletionStatus,
  artifacts: ArtifactEntry[],
