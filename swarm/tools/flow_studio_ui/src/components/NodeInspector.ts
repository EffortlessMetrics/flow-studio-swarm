...
export class NodeInspector {
  ...
  private onSave?: (step: FlowStep) => void;
  private onCancel?: () => void;
  private onError?: (error: Error) => void;

  private stepData: NodeData | null = null;
  private originalStep: FlowStep | null = null;
  private formState: StepFormState | null = null;
   ...
   */
  async init(stepData: NodeData): Promise<void> {
   ...
   */
  private async loadStations(): Promise<StationInfo[]> {
    try {
      ...
      const data = await response.json() as { stations: StationInfo[] };
   ...
   */
  private stepToFormState(step: FlowStep): StepFormState {
   ...
   * Create default form state for new step
   */
  private createDefaultFormState(nodeData: NodeData): StepFormState {
    return {
  ...
  private createForm(): HTMLElement {
    ...
    // Step ID (read-only)
    form.appendChild(this.createReadOnlyField(
      "Step ID",
      this.formState!.stepId,
      "step-id"
    ));

    // Role/Description
    form.appendChild(this.createTextareaField(
      "Role / Description",
      this.formState!.role,
      "role",
      "What this step does...",
      (value) => { this.formState!.role = value; this.clearFieldError("role"); }
    ));

    // Assigned Agents (tag input)
    form.appendChild(this.createTagInputField(
      "Assigned Agents",
      this.formState!.agents,
      "agents",
      this.availableAgents,
      (agents) => { this.formState!.agents = agents; this.clearFieldError("agents"); }
    ));

    // Required Artifacts
    form.appendChild(this.createListEditorField(
      "Required Artifacts",
      this.formState!.requiredArtifacts,
      "required-artifacts",
      "artifact_name.md",
      (artifacts) => { this.formState!.requiredArtifacts = artifacts; }
    ));
    ...
    form.appendChild(this.createListEditorField(
      ...
      "optional-artifacts",
      "optional_artifact.md",
      (artifacts) => { this.formState!.optionalArtifacts = artifacts; }
    ));

    // Teaching Note (only shown in teaching mode or always for editing)
    const teachingNoteSection = this.createTextareaField(
      "Teaching Note",
      this.formState!.teachingNote,
      "teaching-note",
      "Explain what to look for in this step...",
      (value) => { this.formState!.teachingNote = value; }
    );
    if (!getTeachingMode()) {
    ...
    // Is Decision Point
    form.appendChild(this.createCheckboxField(
      "Is Decision Point",
      this.formState!.isDecisionPoint,
      "is-decision",
      "Mark this step as a decision point in the flow",
      (checked) => { this.formState!.isDecisionPoint = checked; }
    ));

    // Station ID (dropdown)
    form.appendChild(this.createDropdownField(
      "Station ID",
      this.formState!.stationId,
      "station-id",
      this.stations.map(s => ({
        value: s.station_id,
   ...
   */
  private createReadOnlyField(label: string, value: string, id: string): HTMLElement {
  ...
  private createTagInputField(
    ...
    const input = field.querySelector(".node-inspector__tag-input-field") as HTMLInputElement;
  ...
  private createListEditorField(
    ...
    const addInput = field.querySelector(".node-inspector__list-add-input") as HTMLInputElement;
  ...
  private async handleSave(): Promise<void> {
    if (!this.formState || !this.stepData) return;
   ...
   */
  isDirty(): boolean {
    if (!this.formState || !this.originalStep) return false;

    return (
      this.formState.role !== (this.originalStep.role || "") ||
 ...
 */
export async function createNodeInspector(
  container: HTMLElement,
  stepData: NodeData,
  options: Omit<NodeInspectorOptions, "container">
