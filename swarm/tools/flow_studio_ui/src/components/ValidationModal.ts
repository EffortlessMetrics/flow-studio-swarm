...
export class ValidationModal {
  ...
  private options: ValidationModalOptions;
  private resolvePromise: ((decision: ValidationDecision) => void) | null = null;
   ...
   */
  async show(result: FlowValidationResult): Promise<ValidationDecision> {
   ...
   */
  close(decision: ValidationDecision = "cancel"): void {
   ...
   */
  private render(result: FlowValidationResult): void {
    ...
    const content = this.modal.querySelector<HTMLElement>(".validation-modal-content");
    if (content) {
   ...
   */
  private renderSummary(result: FlowValidationResult): string {
   ...
   */
  private renderBlockingNotice(): string {
    return `
   ...
   */
  private renderIssuesBySection(issues: ValidationIssue[]): string {
    ...
    for (const severity of ["CRITICAL", "WARNING", "INFO"] as ValidationSeverity[]) {
   ...
   */
  private renderIssue(issue: ValidationIssue): string {
   ...
   */
  private renderActions(hasCritical: boolean, hasWarnings: boolean): string {
    if (hasCritical) {
 ...
 */
export function createValidationModal(options?: ValidationModalOptions): ValidationModal {
