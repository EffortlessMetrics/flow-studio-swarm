...
def discover_all_runs(compute_size: bool = True) -> List[RunInfo]:
    ...
    Args:
        compute_size: Whether to compute directory size (expensive).
                     Defaults to True.
    """
    runs: List[RunInfo] = []
    seen: set[str] = set()
...
def cmd_prune(args: argparse.Namespace) -> int:
    ...
    # Optimization: Skip expensive size calculation during discovery
...
def cmd_quarantine(args: argparse.Namespace) -> int:
    ...
    # Optimization: Skip size calculation
    runs = discover_all_runs(compute_size=False)
