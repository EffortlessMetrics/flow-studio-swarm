...
def build_detailed_json_output(
    ...
    try:
        ...
        # Prefer get_flow_order if available (returns ordered list of flow keys)
        flow_keys = get_flow_order()
    except (ImportError, AttributeError):
        try:
            # Fallback to get_flow_keys if get_flow_order is missing
            from swarm.config.flow_registry import get_flow_keys

            flow_keys = get_flow_keys()
        except ImportError:
            # Last resort fallback if registry not available
            # This should match the standard 7-flow lifecycle
