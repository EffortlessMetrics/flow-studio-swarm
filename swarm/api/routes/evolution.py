...
async def apply_evolution_patch_endpoint(
    ...
    if ":" in request.patch_id:
        run_id_val, patch_id_val = request.patch_id.split(":", 1)
        try:
            run_id = validate_path_component(run_id_val, "run_id")
            patch_id = validate_path_component(patch_id_val, "patch_id")
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
    else:
