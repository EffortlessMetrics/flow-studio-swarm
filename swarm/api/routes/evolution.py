...
def _validate_params(run_id: Optional[str] = None, patch_id: Optional[str] = None):
    try:
...
async def get_run_evolution_patches(run_id: str):
    ...
    """
    _validate_params(run_id=run_id)
...
async def get_evolution_patch_details(
    ...
    Raises:
        404: Patch not found.
    """
    _validate_params(run_id=run_id, patch_id=patch_id)
...
async def apply_evolution_patch_endpoint(
    ...
    if ":" in request.patch_id:
        ...
        _validate_params(patch_id=patch_id)
        run_id = None
