...
async def _write_stop_report(
    ...
    # Sanitize inputs to prevent Stored XSS/Markdown injection
    safe_run_id = html.escape(str(run_id or ""))
    safe_stopped_at = html.escape(str(stop_info.stopped_at or ""))
    safe_reason = html.escape(str(stop_info.stop_reason or ""))
    safe_last_step = html.escape(str(stop_info.last_step_id or "None"))
    safe_flow_id = html.escape(str(state.get("flow_id", "Unknown") or ""))
    safe_status = html.escape(str(state.get("status", "Unknown") or ""))
    ...
    if stop_info.last_routing_intent:
        # Escape backticks to prevent breaking out of code block
        safe_intent = str(stop_info.last_routing_intent).replace("```", "'''")
        lines.extend(
            [
                "## Last Routing Intent",
                "",
    ...
    if stop_info.last_tool_calls:
        ...
        for call in stop_info.last_tool_calls:
            # Replace backticks with single quotes to prevent breaking inline code
