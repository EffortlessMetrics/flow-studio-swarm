...
async def _write_stop_report(
    ...
    if stop_info.last_routing_intent:
        # Sanitize routing intent to prevent breaking out of code block and XSS
        safe_intent = html.escape(stop_info.last_routing_intent.replace("```", "'''"))
        lines.extend(
            [
                "## Last Routing Intent",
                "",
    ...
    if stop_info.last_tool_calls:
        ...
        for call in stop_info.last_tool_calls:
            # Escape backticks to prevent breaking out of code span and XSS
            safe_call = html.escape(call.replace("`", "'"))
