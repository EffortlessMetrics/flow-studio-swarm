...
class FlowStudioConfig:
    ...
    def list_runs(self) -> list[Path]:
        ...
        if not self.runs_dir.exists():
            return []
        # PERFORMANCE: Using os.scandir() to leverage cached OS metadata (is_dir, name) and avoid redundant stat calls
        with os.scandir(self.runs_dir) as entries:
            return sorted(
                Path(entry.path) for entry in entries if entry.is_dir() and not entry.name.startswith(".")
            )
