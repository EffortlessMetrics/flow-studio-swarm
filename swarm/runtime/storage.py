...
def list_all_run_ids(runs_dir: Path = RUNS_DIR) -> List[RunId]:
    """Fast-path directory scanner to list all run candidates.

    Avoids expensive file existence checks. Ignores hidden folders and caches.
    Returns directories sorted chronologically by run ID.

    Args:
        runs_dir: Base directory for runs. Defaults to RUNS_DIR.

    Returns:
        List of directory names that are candidate run IDs, sorted alphabetically.
    """
