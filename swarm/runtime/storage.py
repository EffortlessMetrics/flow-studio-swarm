...
def list_all_run_ids(runs_dir: Path = RUNS_DIR) -> List[RunId]:
    """Fast-path scanner to get all potential run directories.

    Uses os.scandir to retrieve all directory names under runs_dir without
    performing expensive file existence checks. Ignores hidden directories
    and cache directories (starting with '.' or '__').
