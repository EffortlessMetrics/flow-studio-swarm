...
class RunService:
    ...
    def list_runs_paginated(
        ...
        # Get potential runs from disk (fast, no existence checks)
        # Note: This list may include invalid directories which will be filtered
        # during summary creation for the requested page.
        ...
        for rid in sliced_ids:
            ...
            if rid in example_set:
                ...
                # Fallback to legacy check if include_legacy is True and read_summary failed
                if not summary and include_legacy:
                    if storage.is_legacy_run(rid):
                        summary = self._create_legacy_summary(rid, is_example=False)
