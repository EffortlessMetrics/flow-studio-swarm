...
class RunService:
    ...
    def list_runs_paginated(
        ...
        if include_legacy:
            # Fast-path for legacy+active: use scandir without checking file existence
            other_ids_raw = storage.list_all_run_ids()
        else:
            # Must check meta.json to confirm it's an active run and not a legacy run
            other_ids_raw = storage.list_runs()

        # Assumption: run_id lexicographical order ~= chronological order
        other_ids = sorted(other_ids_raw, reverse=True)
