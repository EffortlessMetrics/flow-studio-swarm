...
class RunService:
    ...
    def list_runs_paginated(
        ...
        if include_legacy:
            # Fast path: list all directories without checking existence
            other_ids = storage.list_all_run_ids()
            other_ids.sort(reverse=True)
            legacy_set_check = True
        else:
            # Validated path: must check for meta.json to ensure accurate count
            other_ids = storage.list_runs()
            other_ids.sort(reverse=True)
            legacy_set_check = False
        ...
        for rid in sliced_ids:
            ...
            if rid in example_set:
                ...
                if not summary and legacy_set_check:
                    summary = self._create_legacy_summary(rid, is_example=False)
